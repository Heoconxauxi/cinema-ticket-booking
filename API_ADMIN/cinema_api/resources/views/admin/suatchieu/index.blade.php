        ...
        <div class="col-12">
            <div class="card mb-4">
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table table-striped table-borderless align-items-center mb-0">
                            ...
                            <tbody>
                                @forelse ($list_suatchieu as $item)
                                <tr>
                                    {{-- Cập nhật STT cho đúng với phân trang --}}
                                    <td>{{ $loop->iteration + ($list_suatchieu->currentPage() - 1) * $list_suatchieu->perPage() }}</td>
                                    <td>{{ $item->phim->TenPhim ?? 'Phim đã bị xóa' }}</td>
                                    <td>{{ $item->phong->TenPhong ?? 'Phòng đã bị xóa' }}</td>
                                    <td>{{ $item->GioChieu->format('H:i') }} - {{ $item->GioChieu->format('d/m/Y') }}</td>
                                    <td>{{ $item->NgayTao->format('d/m/Y') }}</td>
                                    <td>
                                        @if ($item->TrangThai == 1)
                                    ...
                                    <td>
                                        <a href="{{ route('admin.suatchieu.edit', $item->MaSuatChieu) }}" class="btn btn-info btn-sm">SỬA</a>
                                        <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" 
                                                data-bs-target="#deleteModal" 
                                                data-id="{{ $item->MaSuatChieu }}" 
                                                data-name="Suất chiếu ({{ $item->phim->TenPhim ?? 'N/A' }} lúc {{ $item->GioChieu->format('H:i d/m') }})">
                                            XOÁ
                                        </button>
                                    </td>
                                </tr>
                ...
                <div class="card-footer">
                    ...
                    {{ $list_suatchieu->appends(request()->query())->links() }}
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Bạn có chắc muốn xóa <strong id="itemName"></strong>?
                <p class="text-danger mt-2">Lưu ý: Hành động này sẽ xóa vĩnh viễn suất chiếu và tất cả vé đã bán (nếu có) của suất chiếu này.</p>
            </div>
...
<script>
    ...
    deleteModal.addEventListener('show.bs.modal', function(event) {
        ...
        var actionUrl = '{{ route("admin.suatchieu.destroy", ":id") }}';
